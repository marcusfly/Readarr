...
export const defaultState = {
  ...
  magazineDefaults: {
    rootFolderPath: '',
    monitor: magazineMonitorOptions[0].key,
    qualityProfileId: 0,
    metadataProfileId: 0,
    tags: []
  }
...
export const ADD_MAGAZINE = 'search/addMagazine';
...
export const SET_MAGAZINE_ADD_DEFAULT = 'search/setMagazineAddDefault';
...
export const addMagazine = createThunk(ADD_MAGAZINE);
...
export const setMagazineAddDefault = createAction(SET_MAGAZINE_ADD_DEFAULT);
...
export const actionHandlers = handleThunks({
  ...
  [ADD_MAGAZINE]: function(getState, payload, dispatch) {
    dispatch(set({ section, isAdding: true }));

    const foreignId = payload.foreignId;
    const items = getState().search.items;
    const itemToAdd = _.find(items, { foreignId });
    const newMagazine = getNewMagazine(_.cloneDeep(itemToAdd.magazine), payload);
...
export const reducers = createHandleActions({
  ...
  [SET_MAGAZINE_ADD_DEFAULT]: function(state, { payload }) {
    const newState = getSectionState(state, section);

    newState.magazineDefaults = {
      ...newState.magazineDefaults,
      ...payload
    };
